...
namespace gazebo
{
    ...
    class RobileControlPlugin : public ModelPlugin
    {
...
namespace gazebo
{
    ...
    RobileControlPlugin::RobileControlPlugin() {}
    ...
    // Load the controller
    void RobileControlPlugin::Load(physics::ModelPtr parent,
            sdf::ElementPtr sdf)
    {
        ...
        {
            ROS_INFO_NAMED("robile_control_plugin", "RobileControlPlugin missing <robotNamespace>, "
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <commandTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <odometryTopic>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <odometryFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <robotBaseFrame>, "
                    "defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <odometryRate>, "
                    "defaults to %f",
        ...
        {
            ROS_WARN_NAMED("robile_control_plugin", "RobileControlPlugin (ns = %s) missing <cmdVelTimeout>, "
                    "defaults to %f",
        ...
        {
            ROS_FATAL_STREAM_NAMED("robile_control_plugin", "RobileControlPlugin (ns = " << robot_namespace_
        ...
        vel_sub_ = nh_->subscribe<geometry_msgs::Twist>(command_topic_, 1,
                &RobileControlPlugin::cmdVelCallback, this);
        odometry_pub_ = nh_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

        // listen to the update event (broadcast every simulation iteration)
        update_connection_ =
            event::Events::ConnectWorldUpdateBegin(
                    boost::bind(&RobileControlPlugin::UpdateChild, this));

        ROS_INFO("Robile plugin loaded");
    }

    // Update the controller
    void RobileControlPlugin::UpdateChild()
    {
    ...
    void RobileControlPlugin::FiniChild()
    {
        nh_->shutdown();
    }

    void RobileControlPlugin::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
    ...
    void RobileControlPlugin::publishOdometry(double step_time)
    {
    ...
    GZ_REGISTER_MODEL_PLUGIN(RobileControlPlugin)
}
